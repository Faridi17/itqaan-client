...
const ArticleCard = ({ id, createdAt, title, summary, cover, customFunc }) => {
    return (
        <article className="p-6 bg-white rounded-lg border border-gray-200 shadow-md md:flex gap-4 items-center">
            ...
            <div className='mt-2 md:w-3/5'>
                ...
                <div className='flex gap-2'>
                    <Link to={`/dashboard/edit/${id}`} className='flex gap-1 items-center text-white bg-teal-600 px-2.5 py-1 text-sm rounded-md'>
                        <FiEdit />
                        Edit
                    </Link>
    ...
    return (
        <Dashboard>
            <div className='mt-20 max-w-6xl mx-auto p-5'>
                ...
                <div className='mt-6'>
                    <Link to='/dashboard/create' className='w-fit bg-primary flex gap-2 items-center text-white px-4 py-1.5 rounded-md'>
                        <FaRegEdit />
                        Create Blog
                    </Link>
                </div>
                <div className='flex flex-wrap gap-4 mt-8'>
                    {blogs.map((blog, index) => (
                        <ArticleCard key={index} {...blog} customFunc={deleteButton} />
                    ))}
                </div>
            </div>
                    ...
                    <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 justify-center items-center w-[24rem]">
                        <div className="relative p-4 w-full max-w-md h-full md:h-auto">
                            <div className="relative p-4 text-center bg-white rounded-lg shadow sm:p-5">
                                <button onClick={() => setModal(false)} type="button" className="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-800 transition-all rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="deleteModal">
                                ...
                                <p className="mb-4 text-gray-700 opacity-95">Are you sure you want to delete this blog?</p>
                                <div className="flex justify-center items-center space-x-4">
                                    <button onClick={() => setModal(false)} type="button" className="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10">
                                        No, cancel
                                    </button>
                                    <button onClick={deleteBlog} type="submit" className="py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300">
                                        Yes, I'm sure
                                    </button>
                                </div>
                            </div>
